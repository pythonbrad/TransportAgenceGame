...
set engin_list "Engin1 Engin2 Engin3 Engin4 Engin5 Engin6 Engin7 Engin8"
...
proc add_car {} {
	global travel_selected color_selected engin_selected
...
bind . <Control-F12> {console show}
bind . <+> {if {[wm overrideredirect .]} {wm overrideredirect . 0} else {wm overrideredirect . 1}}

place [ttk::frame .view_navig] -x 100 -y 200
pack [ttk::button .button_yview_up -text UP -command {.c yview scroll 1 units}] -in .view_navig -side top
pack [ttk::button .button_yview_down -text DOWN -command {.c yview scroll -1 units}] -in .view_navig -side bottom
pack [ttk::button .button_xview_up -text LEFT -command {.c xview scroll 1 units}] -in .view_navig -side left
pack [ttk::button .button_xview_down -text RIGHT -command {.c xview scroll -1 units}] -in .view_navig -side right
...
if {$simu} {
	...
	bind . <B3-Motion> {get_coord_scanner2 %x %y}
		...
		foreach travel $travel_list {
			.myMenubutton1.myMenu add command -label "$travel" -command "set travel_selected {$travel}"
		}
		...
		foreach engin $engin_list {
			.myMenubutton3.myMenu add command -label "$engin" -command "set engin_selected {$engin}"
		}
		pack .myMenubutton3 -side left -in .frame
		
		pack [ttk::button .button0 -text "En route" -command {add_car}] -side left -padx 2 -in .frame
	}
